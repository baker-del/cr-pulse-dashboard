...
SQL_PIPELINES  = {'default', '757781604'}
...
def _week_start(dt):
    """Monday of dt's ISO week."""
    return (dt - timedelta(days=dt.weekday())).replace(
        hour=0, minute=0, second=0, microsecond=0, tzinfo=timezone.utc)

def _week_label(ws, q_start):
    """'Apr 1' label using the later of week_start or quarter_start."""
...
def load_weekly_cohorts(quarter: str, year: int):
    """
    Returns dict with keys 'sal' and 'aec', each a list of weekly rows
    covering only weeks within the selected quarter.
    """
    ...
    mql_weeks = defaultdict(lambda: {
        'sal': 0, 'aec': 0, 'total': 0,
        'sal_sal': 0, 'aec_sal': 0,          # BDR-accepted contacts
        'sources': Counter(),
    })
    ...
    # ── Deal weekly buckets — only deals CREATED within the quarter
    ...
    # ── Build sorted week list — only weeks that overlap the quarter
    all_weeks = sorted(set(list(mql_weeks.keys()) + list(deal_weeks.keys())))
    # Keep only weeks whose Monday is before quarter end (handles boundary)
